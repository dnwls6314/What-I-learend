...
def train(model, trn_loader, device, criterion, optimizer, batch_size, model_type):
    """ Train function
    ...
    for batch_idx, batch in enumerate(trn_loader) :
        ...
        # input sequence를 원핫벡터로 변경
        x = one_hot_encoding(x)
        x = x.to(device); label = label.to(device)
        if model_type=='RNN' :
            hidden = tuple([each.data for each in hidden])[0].reshape(model.n_layer, 100, model.hidden_size)
        else :
...
@torch.no_grad()
def validate(model, val_loader, device, criterion, batch_size, model_type='RNN'):
    """ Validate function
...
#############################################################     LSTM start   ################################################
    ...
    for epoch in range(epochs) :
        ...
        lstm_train_loss.append(train_loss); lstm_val_loss.append(test_loss)
        
        
    lstm_generated_text = generate(lstm_model, 'The', 5, 'LSTM', train_dataset.char2int, train_dataset.int2char)
    lstm_text = open('lstm2.txt', 'w')
    lstm_text.write(lstm_generated_text)
    lstm_text.close()
#############################################################     LSTM end   ################################################    
    
    # draw loss plot
    draw_result_plot(rnn_train_loss, rnn_val_loss, lstm_train_loss, lstm_val_loss)
