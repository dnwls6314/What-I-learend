...
def cosine_kernel(x, train, h):
    ...
    # h: bandwidth
    # return p(x)
    a_ = [(x-point)/h for point in train]
    kernel_output = []
    
    for a in a_ :
        if np.abs(a) <= 1 :
            kernel_output.append((np.pi/4)*np.cos((np.pi/2)*a))
        else :
...
def gaussian_kernel(x, train, h):
    ...
    for a in a_:
        kernel_output.append(norm.pdf(a))
...
def gaussian_2d_kernel(x, train, h):
    ...
    mean = np.array([0,0])
    kernel_output = multivariate_normal.pdf(a_, mean=mean)
...
def epanechnikov_kernel(x,train,h):
    ...
    for a in a_:
        if np.abs(a) <= 1:
            kernel_output.append((3/4)*(1-a**2))
        else :
            kernel_output.append(0)
