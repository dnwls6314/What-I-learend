...
def cosine_kernel(x, train, h):
    ...
    # h: bandwidth
    # return p(x)
    u_ = [(x-point)/h for point in train]
    kernel_output = []
    
    for u in u_ :
        if np.abs(u) <= 1 :
            kernel_output.append((np.pi/4)*np.cos((np.pi/2)*u))
        else :
            kernel_output.append(0)
    
    return 1/len(train) * np.sum(np.multiply((1/h), kernel_output))
...
def gaussian_kernel(x, train, h):
    ...
    for u in u_:
        kernel_output.append(norm.pdf(u))
        
    return 1/len(train)*np.sum(np.multiply(kernel_output,(1/h)))    
...
def gaussian_2d_kernel(x, train, h):
    ...
    mean = np.array([0,0])
    kernel_output = multivariate_normal.pdf(u_, mean=mean)
    
    return 1/len(train)*np.sum(np.multiply(kernel_output,(1/h**2)))
...
def epanechnikov_kernel(x,train,h):
    ...
    for u in u_:
        if np.abs(u) <= 1:
            kernel_output.append((3/4)*(1-u**2))
        else :
            kernel_output.append(0)
